...
- Runs on **Bun** (≥1.3.12, pinned to 1.4.2 — `Bun.WebView` needs 1.3.12+; the pin lives in `.bun-version` and `packageManager` in `package.json`). Bun runs the TS directly; there is no `tsx`, no build, no bundler, no test runner.
